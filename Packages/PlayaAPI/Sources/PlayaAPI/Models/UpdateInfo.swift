...
public struct UpdateInfo: Codable, Hashable, Sendable {
    ...
    public let mv: FileUpdateInfo?

    public init(art: FileUpdateInfo? = nil, camps: FileUpdateInfo? = nil, events: FileUpdateInfo? = nil, mv: FileUpdateInfo? = nil) {
        self.art = art
        self.camps = camps
        self.events = events
        self.mv = mv
...
public extension UpdateInfo {
    ...
    var lastUpdated: Date? {
        [art?.updated, camps?.updated, events?.updated, mv?.updated]
            .compactMap { $0 }
            .max()
    }
