...
## Part C — Pre-populated YapDatabase seed (RETIRED 2026-09-12)

> **Obsolete.** YapDatabase was removed from the app in the 2026-09-12 Yap-removal work
> (`Docs/2026-09-12-yap-removal-and-playa-bug-fixes.md`, phases 3/4). There is no Yap
> database, no `BRCDataImporter`, and no `iBurn/iBurn-2026.zip` any more — the step that
> regenerated that zip is gone from the data-refresh procedure. The only seed the app
> ships is the PlayaDB one (`iBurn/PlayaDB-<year>.zip` +
> `iBurnWatch/PlayaDB-<year>.zip`), regenerated with
> `swift run --package-path Packages/PlayaSeed playa-seed --fetch-media`.
>
> The `iBurn/iBurn-*.zip` `.gitignore` rule is left in place so any stale local copy stays
> untracked. The historical procedure is preserved in git history.
