...
/// asks here first.
///
/// The verdict is the shared `LocationEmbargo` rule — nothing is decided here,
/// this type only sources the three inputs. A tier unlocks when one of these
/// holds:
///
...
///    of its own. The single bypass, for every tier.
/// 2. **The tier's date**, read from the same `YearSettings.plist` the phone
///    uses (a shared resource of both app targets) and evaluated live on every
///    call — on its own for the camp tier, and paired with a GPS fix inside the
///    Burning Man region for the art tier.
///
/// For art, the date on its own is explicitly not enough: the watch clock is
...
/// phone shows no art placement all season. Camp addresses were relaxed to a
/// date-only unlock on 2026-08-22 so the week-early release works before travel.
///
...
enum WatchEmbargo {
    ...
    /// the art tier's rule. It unlocks nothing by itself — the tier's date still
    /// has to arrive — so this can run years before the event with no effect.
