...
class EventViewModel: ObservableObject, DataViewModel {
    ...
    init?(data: BRCData, metadata: BRCMetadata) {
    ...
    func appear() async {
        let location = await Self.locationInfo(event: event)
        hostName = location.camp?.name ?? location.art?.name ?? event.locationName
        
        let host = location.camp ?? location.art ?? event
        locationDescription = Self.locationDescription(for: host)
    }
    
    static func locationInfo(event: BRCEvent) async -> EventLocation {
        await withCheckedContinuation { continuation in
            BRCDatabaseManager.shared.uiConnection.read { transaction in
                continuation.resume(
                    returning: EventLocation(
                        camp: event.camp(from: transaction),
                        art: event.art(from: transaction)
                    )
                )
            }
        }
